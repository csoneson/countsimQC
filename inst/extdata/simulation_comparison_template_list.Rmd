...
obj <- lapply(ddsList, function(ds) {
  ...
  if (ncol(dge) > maxNForDisp) {
    ...
    destmp <- destmp[, colSums(destmp) > 0]
...
sampleCorrDF <- lapply(obj, function(x) {
...
sampleCorrDF <- do.call(rbind, sampleCorrDF) %>% 
...
featureCorrDF <- lapply(obj, function(x) {
  cpms <- edgeR::cpm(x$dge, prior.count = 2, log = TRUE)
...
featureCorrDF <- do.call(rbind, featureCorrDF) %>% 
...
makeDF <- function(df, column) {
  if (length(unique(df$dataset)) > 1) {
    ...
    ks_df <- data.frame(t(combn(unique(df$dataset), 2)), stringsAsFactors = FALSE) %>%
    ...
    ks_res <- as.data.frame(t(sapply(1:nrow(ks_df), function(i) {
      ...
      ## Calculate area between ecdfs
      e1 <- ecdf(df[, column][df$dataset == ks_df$dataset1[i]])
      e2 <- ecdf(df[, column][df$dataset == ks_df$dataset2[i]])
      xv <- sort(unique(df[, column]))
      ediff <- abs(e1(xv) - e2(xv))
      earea <- caTools::trapz((xv - min(xv))/(max(xv) - min(xv)), ediff)
      ## Calculate runs test
      dftmp <- df %>% dplyr::filter(dataset %in% c(ks_df$dataset1[i], ks_df$dataset2[i])) %>%
        dplyr::arrange_(column)
      runs_res <- randtests::runs.test(as.numeric(dftmp$dataset == ks_df$dataset1[i]), 
                                       threshold = 0.5, alternative = "left.sided",
                                       plot = FALSE)
      
      c(statistic = round(kst$statistic, 3), pvalue = round(kst$p.value, 3),
        diffarea = round(earea, 3), runsstatistic = round(runs_res$statistic, 3),
        runspvalue = round(runs_res$p.value, 3))
    })))
    ks_df$`K-S statistic` <- ks_res$statistic
    ks_df$`K-S p-value` <- ks_res$pvalue
    ks_df$`Scaled area between ECDFs` <- ks_res$diffarea
    ks_df$`Runs statistic` <- ks_res$runsstatistic
    ks_df$`Runs p-value` <- ks_res$runspvalue
...
The following model formulae were used to calculate dispersions for the
different data sets.
...
The relation between the empirical mean and variance. The difference between 
these mean-variance plots and the mean-dispersion plots above is that the plots 
in this section do not take the information about the experimental design and 
sample grouping into account, but simply display the mean and variance of
log2(CPM) estimates across all samples, calculated using the `cpm` function from
[edgeR](https://bioconductor.org/packages/release/bioc/html/edgeR.html) 
...
The distribution of the total count per sample multiplied by the TMM
