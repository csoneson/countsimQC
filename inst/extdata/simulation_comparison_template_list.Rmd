...
sampleCorrDF <- do.call(rbind, lapply(obj, function(x) {
  ...
  if (ncol(cpms) > 500) {
    set.seed(123)
    cpms <- cpms[, sample(seq_len(ncol(cpms)), 500, replace = FALSE)]
  }
  corrs <- cor(cpms, use = "pairwise.complete.obs", method = "spearman")
  data.frame(
    Correlation = corrs[upper.tri(corrs)]
  )
})) %>% dplyr::mutate(dataset = rep(names(obj), sapply(obj, function(x) getNForCorr(ncol(x$dge), 500))))
```
...
featureCorrDF <- do.call(rbind, lapply(obj, function(x) {
  cpms <- edgeR::cpm(x$dge, prior.count = 2, log = TRUE)
  cpms <- cpms[rowVars(cpms) > 0, ]
  if (nrow(cpms) > 500) {
    set.seed(123)
    cpms <- cpms[sample(seq_len(nrow(cpms)), 500, replace = FALSE), ]
  }
  corrs <- cor(t(cpms), use = "pairwise.complete.obs", method = "spearman")
  data.frame(
    Correlation = corrs[upper.tri(corrs)]
  )
})) %>% dplyr::mutate(dataset = rep(names(obj), sapply(obj, function(x) getNForCorr(nrow(x$dge), 500))))
```
...
with a prior.count of 2. If there are more than 500 samples in a data set, the
pairwise correlations between 500 randomly selected samples are shown.
...
there are more than 500 such variables in a data set, the pairwise correlations
between 500 randomly selected variables are shown.
