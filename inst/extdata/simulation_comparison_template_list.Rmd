...
knitr::opts_chunk$set(echo = show_code, warning = FALSE,
                      fig.path = paste0(gsub(".html", "_figures/", output_file)))
...
```{r bcv_edgeR, fig.width = 3.5 * length(dds_list)}
...
```{r dispersion_DESeq2, fig.width = 3.5 * length(dds_list)}
ggplot(feature_df %>% dplyr::arrange(baseMean), 
       aes(x = baseMean, y = dispGeneEst)) + geom_point(size = 0.25) + 
  facet_wrap(~dataset, nrow = 1) + scale_x_log10() + scale_y_log10() +  
...
```{r libsize_sephist, fig.width = 3.5 * length(dds_list)}
...
ggplot(sample_df, aes(x = Libsize, group = dataset, color = dataset)) + 
  stat_ecdf() + ylab("Cumulative proportion") + 
  xlab("Library size") + thm
```
...
```{r tmm_sephist, fig.width = 3.5 * length(dds_list)}
...
```{r logcpm_sephist, fig.width = 3.5 * length(dds_list)}
...
```{r fraczerosample_sephist, fig.width = 3.5 * length(dds_list)}
...
```{r fraczerofeaturesephist, fig.width = 3.5 * length(dds_list)}
...
```{r logcpmfraczero_sepscatter, fig.width = 3.5 * length(dds_list)}
...
```{r libsizefraczero_sepscatter, fig.width = 3.5 * length(dds_list)}
