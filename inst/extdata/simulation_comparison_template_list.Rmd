...
These bar plots show the number of samples (columns) and variables (rows) in
each data set.
...
`r I(defineTableDesc(calculateStatistics = calculateStatistics, subsampleSize = subsampleSize, kfrac = kfrac, kmin = kmin, obstype = "feature", aspect = "average log CPM and tagwise dispersion", minvalue = 0, maxvalue = 1, permutationPvalues = permutationPvalues, nPermutations = nPermutations)$tabledesc2d)`


```{r BCVedgeRTests}
if (calculateStatistics)
  makeDF(df = featureDF, column = c("AveLogCPM", "Tagwise"), 
...
`r I(defineTableDesc(calculateStatistics = calculateStatistics, subsampleSize = subsampleSize, kfrac = kfrac, kmin = kmin, obstype = "feature", aspect = "base mean and genewise dispersion", minvalue = 0, maxvalue = 1, permutationPvalues = permutationPvalues, nPermutations = nPermutations)$tabledesc2d)`

```{r dispersionDESeq2Tests}
if (calculateStatistics)
  makeDF(df = featureDF, column = c("baseMean", "dispGeneEst"), 
         permutationPvalues = permutationPvalues, nPermutations = nPermutations, 
         subsampleSize = subsampleSize, seed = seed, kmin = kmin, kfrac = kfrac)
```
...
the variables. The difference between these mean-variance plots and the 
...
`r I(defineTableDesc(calculateStatistics = calculateStatistics, subsampleSize = subsampleSize, kfrac = kfrac, kmin = kmin, obstype = "feature", aspect = "average and variance of the log CPM", minvalue = 0, maxvalue = 1, permutationPvalues = permutationPvalues, nPermutations = nPermutations)$tabledesc2d)` 
...
`r I(defineTableDesc(calculateStatistics = calculateStatistics, subsampleSize = subsampleSize, kfrac = kfrac, kmin = kmin, obstype = "sample", aspect = "library size", minvalue = min(sampleDF$Libsize), maxvalue = max(sampleDF$Libsize), permutationPvalues = permutationPvalues, nPermutations = nPermutations)$tabledesc)`
...
`r I(defineTableDesc(calculateStatistics = calculateStatistics, subsampleSize = subsampleSize, kfrac = kfrac, kmin = kmin, obstype = "sample", aspect = "TMM factor", minvalue = min(sampleDF$TMM), maxvalue = max(sampleDF$TMM), permutationPvalues = permutationPvalues, nPermutations = nPermutations)$tabledesc)`
...
`r I(defineTableDesc(calculateStatistics = calculateStatistics, subsampleSize = subsampleSize, kfrac = kfrac, kmin = kmin, obstype = "sample", aspect = "effective library size", minvalue = min(sampleDF$EffLibsize), maxvalue = max(sampleDF$EffLibsize), permutationPvalues = permutationPvalues, nPermutations = nPermutations)$tabledesc)`
...
the variables. The abundances are log CPM values calculated by `edgeR`.
...
`r I(defineTableDesc(calculateStatistics = calculateStatistics, subsampleSize = subsampleSize, kfrac = kfrac, kmin = kmin, obstype = "feature", aspect = "average log CPM", minvalue = min(featureDF$AveLogCPM), maxvalue = max(featureDF$AveLogCPM), permutationPvalues = permutationPvalues, nPermutations = nPermutations)$tabledesc)`
...
`r I(defineTableDesc(calculateStatistics = calculateStatistics, subsampleSize = subsampleSize, kfrac = kfrac, kmin = kmin, obstype = "sample", aspect = "fraction zeros", minvalue = min(sampleDF$Fraczero), maxvalue = max(sampleDF$Fraczero), permutationPvalues = permutationPvalues, nPermutations = nPermutations)$tabledesc)`
...
variable (row) in the count matrices.
...
`r I(defineTableDesc(calculateStatistics = calculateStatistics, subsampleSize = subsampleSize, kfrac = kfrac, kmin = kmin, obstype = "feature", aspect = "fraction zeros", minvalue = min(featureDF$Fraczero), maxvalue = max(featureDF$Fraczero), permutationPvalues = permutationPvalues, nPermutations = nPermutations)$tabledesc)`
...
function from `edgeR`, with a prior.count of 2. If there are more than `r
maxNForCorr` samples in a data set, the pairwise correlations between `r
maxNForCorr` randomly selected samples are shown.
...
pairs of variables, calculated from the log(CPM) values obtained via the `cpm`
function from `edgeR`, with a prior.count of 2. Only non-constant variables are
considered, and if there are more than `r maxNForCorr` such variables in a
...
`r I(defineTableDesc(calculateStatistics = calculateStatistics, subsampleSize = subsampleSize, kfrac = kfrac, kmin = kmin, obstype = "feature pair", aspect = "Spearman correlation", minvalue = min(featureCorrDF$Correlation), maxvalue = max(featureCorrDF$Correlation), permutationPvalues = permutationPvalues, nPermutations = nPermutations)$tabledesc)`
...
`r I(defineTableDesc(calculateStatistics = calculateStatistics, subsampleSize = subsampleSize, kfrac = kfrac, kmin = kmin, obstype = "sample", aspect = "library size and fraction of zeros", minvalue = 0, maxvalue = 1, permutationPvalues = permutationPvalues, nPermutations = nPermutations)$tabledesc2d)` 
...
fraction of zeros observed per variable. The abundance is defined as the
...
`r I(defineTableDesc(calculateStatistics = calculateStatistics, subsampleSize = subsampleSize, kfrac = kfrac, kmin = kmin, obstype = "feature", aspect = "average log CPM and fraction of zeros", minvalue = 0, maxvalue = 1, permutationPvalues = permutationPvalues, nPermutations = nPermutations)$tabledesc2d)` 
